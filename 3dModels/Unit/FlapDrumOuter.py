...
def bar_top(angle, screw: bool = False):
    """
    Create the outer "bar top" geometry for a split-flap drum
    Parameters
    ----------
    angle : float
        Rotation angle (degrees) applied about the Z axis to position the bar
        around the drum.
    screw : bool, optional
        If True, add a screw tab/profile. Defaults to False.
    Returns
    """
    spline_points = [
        helpers.calc_position(
            c.DRUM_DIAMETER / 2 - SCREW_DISTANCE * 0.5, SCREW_ROTATION_ANGLE * -1.8
        ),
        helpers.calc_position(
            c.DRUM_DIAMETER / 2 - SCREW_DISTANCE * 1.25, SCREW_ROTATION_ANGLE * -1.8
        ),
        helpers.calc_position(
            c.DRUM_DIAMETER / 2 - SCREW_DISTANCE * 1.3, SCREW_ROTATION_ANGLE * -0.4
        ),
    ...
    if screw:
        result = (
            result.workplaneFromTagged(f"baseScrew{angle}")
            .transformed(
            ...
            .lineTo(
                *helpers.calc_position(
                    c.DRUM_DIAMETER / 2 + c.DRUM_THICKNESS, SCREW_ROTATION_ANGLE * -2
                )
            )
            .lineTo(
                *helpers.calc_position(c.DRUM_DIAMETER / 2, SCREW_ROTATION_ANGLE * -2)
            )
            ...
            .moveTo(
                *helpers.calc_position(
                    c.DRUM_DIAMETER / 2 - SCREW_DISTANCE, -SCREW_ROTATION_ANGLE
                )
            )
...
flapDrumOuter = flapDrumOuter + bar_top(0, screw=True)
flapDrumOuter = flapDrumOuter + bar_top(90)
flapDrumOuter = flapDrumOuter + bar_top(180, screw=True)
flapDrumOuter = flapDrumOuter + bar_top(270)
