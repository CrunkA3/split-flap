...
def screwConnection(self, angle):
    sPnts = [
        helpers.calc_position(c.DRUM_DIAMETER/2 - c.SCREW_DISTANCE*0.5, c.SCREW_ROTATION_ANGLE*1.6),
        helpers.calc_position(c.DRUM_DIAMETER/2 - c.SCREW_DISTANCE*1.25, c.SCREW_ROTATION_ANGLE*1.6),
        helpers.calc_position(c.DRUM_DIAMETER/2 - c.SCREW_DISTANCE*1.75, c.SCREW_ROTATION_ANGLE*0.3),
    ...
    result = (
        ...
        .workplane()
        .transformed(rotate=(0,0,angle))
        .moveTo(*helpers.calc_position(c.DRUM_DIAMETER/2 - c.SCREW_DISTANCE, -c.SCREW_ROTATION_ANGLE))
        .cboreHole(c.SCREW_DIAMETER, c.SCREW_BORE_DIAMETER, c.SCREW_BORE_DEPTH)
    )
    return result

cq.Workplane.barTop = barTop
cq.Workplane.screwConnection = screwConnection
...
motor_bolt = (
    cq.Workplane(
        "front",
        origin=(0,0,c.WHEEL_THICKNESS + c.CENTER_HEIGHT)
    )
...
show_object(
    motor_bolt,
    name="Motor Bolt",
    options=dict(alpha=0.3, color='green'))
